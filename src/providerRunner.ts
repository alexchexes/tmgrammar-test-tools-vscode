...
export async function runGrammarProvider(
  context: ProviderTemplateContext,
  settings: ProviderExecutionSettings
): Promise<GrammarContribution[]> {
  const resolvedCommand = resolveCommandTemplate(settings.command, context)
  const resolvedCwd = resolveProviderCwdTemplate(context, settings.cwd)
